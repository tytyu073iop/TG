...
\begin{document}
  ...
  \end{theorem*}
  \includegraphics{gw.png}
  \begin{proof}
    $\leftarrow$ Если $D'$ - графическая, то существует граф G, который является реализующей $D'$ то есть степенная последовательность графа G совпадает с $D'$
    
    \includegraphics{gpp1.png}

    $\rightarrow$ Пусть $D'$ - графическая последовательность.

    $v_1,v_2,\dots,v_n$ - вершины соответствующего графа.

    Если в этом графе вершина $v_1$ смежна с $v_2,v_3,\dots,v_{d_1+1}$,
    то граф $G'$ реализующий d, можно получить, удалив вершину $v_1$ и все инцедентные ей рёбра.

    Предположим что это не так. Среди всех реализаций последовательности d, обладающих указанным свойством, выберем тот граф $G$, для которого:

    \includegraphics{gpp2.png}
  \end{proof}

  \textbf{Алгоритм проверки последовательности на графичность}

  \textbf{Вход:} последовательность
  
  \textbf{Выход:} Ответ, да либо неотрицательности
  ...
  \begin{proof}
    ...
    \end{enumerate}
  \end{proof}
  \section{Двудольные графы. Теорема Кёнига}
  \begin{definition*}
    Граф называется \textbf{двудольным} если существует разбиение $V(G)=A\cup B(A\cap B=\emptyset)$ на 2 части $A$ и $B$ (называемые долями), такое что для любого ребра ув верно:
    либо $u\in A,v\in B$, либо наоборот.

    \textit{Примечание:} Одна и только одна из долей может быть пустой
  \end{definition*}
  \begin{theorem*}
    \textbf{(Кёнига).} Следующие утверждения эквивалентны
    \begin{enumerate}
      \item двудольный граф
      \item не содержит циклов нечётных длин
      \item не содержит простых циклов нечётной длинны
      \item не содержит простых порождённых циклов нечётной длинны
    \end{enumerate}
    \begin{proof}
      $$1\rightarrow2\rightarrow3\rightarrow4\rightarrow1$$
      $1\rightarrow2$

      G - двудольный граф и с = $(v_1,\dots,v_k, v_1)$ - цикл в графе G
      ...
      Пусть G не содержит простых порождённых циклов нечётной длинны.

      Без ограничения общности можно считать, что G - связен

      Предположим, что граф G не является двудольным

      Зафиксируем вершниу $0\in V(G)$ и построим множества:
      $$A=\{x\in V(G)|d(0,x)-\text{чётное}\}, B=\{x\in V(G)|d(0,x)-\text{нечётное}\}$$
      Покажем, что A и B - доли. 
      $\exists uv\in E(G)$ такое, что $u,v\in A\lor u,v\in B$

      $P_1$ - кратчайшая цепь от 0 до u

      $P_2$ - кратчайшая цепь от 0 до V

      w - последняя вершина в которой цепи совпадают

      \includegraphics{241.png}

      цикл С является простым

      $p+l_1,p+l_2$ - одновременно либо нечётные, либо одновременно чётные.

      $l_1+l_2\text{ - чётное число}\rightarrow l_1+l_2+1 = C\text{ - Нечётное}$

      $C$ - простой цикл нечётной длинны

      Получается противоречие с 4.
    \end{proof}
    \textbf{Алгоритм}
    \begin{enumerate}
      \item Ищём в ширину
      \item если все четные метки несмежны друг с другом и нечетные метки также несмежны друг с другом, то двудольный, иначе нет.
    \end{enumerate}

  \end{theorem*}
  %\chapter{Деревья}
  \section{Определение дерева. Основные свойства деревьев}
  \begin{definition*}
    Связный граф который не содержит циклов называется \textbf{деревом}

    Граф не содержащий циклов \textbf{ациклический} или \textbf{лес}

    \textit{Примечание:} компоненты связности леса - деревья
  \end{definition*}
  \begin{theorem*}
    ...
    \begin{enumerate}
      ...
      \item G не содержит циклов и $m=n-1$
      \item для любых двух несовпадающих вершин существует \textbf{единственная} простая цепь
      \item G не содержит циклов и для любых 2 несмежных вершин $u,v$ граф $G+uv$ имеет ровно один цикл.
    \end{enumerate}
    \begin{proof}
      $$1\rightarrow2\rightarrow3\rightarrow4\rightarrow1$$
      $1\rightarrow2$

      G - дерево $\rightarrow$ G - связен
      ...
      \begin{enumerate}
        \item n = 1, очевидно(тривиальное дерево)
        \item n = 2
        \item Пусть G - дерево порядка $1\leq k\le n$. Тогда $|E(G)|=k-1$
      \end{enumerate}
      ...
      От противного: предположим, что G содержит цикл C и $e\in E(C)$
